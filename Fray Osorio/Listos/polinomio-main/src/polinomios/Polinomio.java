...
public class Polinomio {
    ...
    public void mostrar(JLabel lbl) {
        String espacio = "&nbsp;";
        String linea1 = "";//Exponentes
        String linea2 = "";//Coeficientes
        Nodo apuntador = cabeza;
        while (apuntador != null) {
            String texto = "";
            String texto2 = " ";

            if (apuntador.exponente == 0 && apuntador.coeficiente >= 0) {//Este en negativo
                texto = "+ " + String.valueOf(apuntador.coeficiente);
                texto2 = "  ";
            }else if (apuntador.exponente == 0 && apuntador.coeficiente >= 0) {//Este en negativo
                texto = String.valueOf(apuntador.coeficiente);
                texto2 = "  ";
            }else if (apuntador.coeficiente == 1) {
                texto = "+ " + " X";
            }else if ( apuntador.coeficiente ==0){
                texto = "+ " + String.valueOf(apuntador.coeficiente) + " X";
            }
            else if (apuntador.coeficiente >= 2) {
                texto = "+ " + String.valueOf(apuntador.coeficiente) + " X";
            } else {
                texto = "  "+String.valueOf(apuntador.coeficiente) + " X";
            }
            ...
            if (apuntador.exponente != 0 && apuntador.exponente != 1) {
                texto2 = String.valueOf(apuntador.exponente);
            }else if(apuntador.exponente == 1){
                texto2 = "  ";
            }
            ...
            linea2 += String.format("%0" + texto.length() + "d", 0).replace("0", "   ");
            linea1 += texto2;
            apuntador = apuntador.siguiente;
        }
        if (!linea2.equals("")) {
